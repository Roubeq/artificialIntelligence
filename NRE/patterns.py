...
def handle_weather(match, user_id=None):
    ...
    if user_id and "Ошибка" not in weather_info:
        try:
            temp_match = re.search(r"Температура: ([-+]?\d+)", weather_info)
            wind_match = re.search(r"Ветер: ([-+]?\d+\.?\d*)", weather_info)
...
patterns = [
    ...
    (re.compile(r".*", re.IGNORECASE), handle_weather),
